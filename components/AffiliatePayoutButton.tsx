  ...
  return (
    <>
      ...
      {show && (
        ...
        >
          <div className="bg-white dark:bg-[#111114] rounded-2xl border border-gray-200 dark:border-[#25252D] w-full max-w-sm p-6">
            <div className="text-center mb-5">
              <div className="text-3xl mb-2">💸</div>
              <h2 className="text-lg font-bold text-gray-900 dark:text-[#F5F5F7]">Zleć wypłatę</h2>
              <p className="text-sm text-gray-600 dark:text-[#9A9AA3] mt-1">
                Dostępne środki: <strong className="text-gray-900 dark:text-[#F5F5F7]">{available.toLocaleString('pl-PL', { minimumFractionDigits: 2 })} zł</strong>
              </p>
            </div>

            <div className="mb-4">
              <label className="block text-xs text-gray-600 dark:text-[#9A9AA3] mb-1.5">Kwota (zł)</label>
              <div className="relative">
                <input
                  ...
                  className="w-full bg-gray-100 dark:bg-[#15151A] border border-gray-200 dark:border-[#25252D] rounded-xl px-4 py-3 text-lg font-bold text-center text-gray-900 dark:text-[#F5F5F7] focus:outline-none focus:ring-2 focus:ring-violet-500 dark:focus:ring-[#7C5CFC]"
                />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-[#555] font-medium">zł</span>
              </div>
            </div>

            {error && (
              <p className="text-sm text-red-600 bg-red-50 border border-red-200 dark:bg-red-950/30 dark:border-red-900/50 dark:text-red-400 rounded-xl px-3 py-2 text-center mb-4">{error}</p>
            )}
